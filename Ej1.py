...
def mutate(individual, mutation_prob, algorithm = 'a'):
		if algorithm == 'a':
			upper_x1 = 45			
			upper_x2 = 22
		elif algorithm == 'b':
		...
		if random.random() < mutation_prob:
				...
				if index == 0:
						individual = (random.uniform(0, upper_x1), individual[1])
				else:
						individual = (individual[0], random.uniform(0, upper_x2))
		return individual


def genetic_algorithm(population_size, num_generations, mutation_probability, is_valid, fitness_function, algorithm = 'a'):
	...
	for generation in range(num_generations):
			...
			for _ in range(population_size // 2):
					...
					child1 = mutate(child1, mutation_probability, algorithm)
					child2 = mutate(child2, mutation_probability, algorithm)
		...
		# Imprimir log
			...
			print("Mejor individuo:", max(population, key=lambda ind: fitness_function(ind[0], ind[1])))
...
best_individual = genetic_algorithm(population_size, num_generations, mutation_probability, is_valid_A, fitness_function_A, 'a')
...
num_generations = 1000
population_size = 100
best_individual = genetic_algorithm(population_size, num_generations, mutation_probability, is_valid_b, fitness_function_b, 'b')
...
num_generations = 100
population_size = 100
best_individual = genetic_algorithm(population_size, num_generations, mutation_probability, is_valid, fitness_function, 'c')
